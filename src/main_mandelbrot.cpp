...
//    float sizeX = 2.0f;
...

    {
        ...
        std::cout << "    Real CPU iterations fraction: " << 100.0 * realIterationsFraction / (width * height) << "%" << std::endl;
...

    {
        ...
        gpu::gpu_mem_32f gpuOutput;
        size_t outputSize = width * height;
        gpuOutput.resizeN(outputSize);

        unsigned int workGroupSize = 128;
        unsigned int global_work_size = (outputSize + workGroupSize - 1) / workGroupSize * workGroupSize;
        timer t;
        for (int i = 0; i < benchmarkingIters; ++i) {
            kernel.exec(gpu::WorkSize(workGroupSize, global_work_size),
                        gpuOutput, width, height,
                        centralX - sizeX / 2.0f, centralY - sizeY / 2.0f,
                        sizeX, sizeY,
                        iterationsLimit, 0);
            t.nextLap();
        ...
        gpuOutput.readN(gpu_results.ptr(), outputSize);
        ...
        std::cout << "    Real GPU iterations fraction: " << 100.0 * realIterationsFraction / (width * height) << "%" << std::endl;
    }
